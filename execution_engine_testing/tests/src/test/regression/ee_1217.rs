...
use casper_types::{runtime_args, system::auction, ApiError, RuntimeArgs, U512};

const CONTRACT_REGRESSION: &str = "ee_1217_regression.wasm";
const CONTRACT_ADD_BID: &str = "add_bid.wasm";

const PACKAGE_NAME: &str = "call_auction";
const CONTRACT_CALL_AUCTION_ENTRYPOINT_SESSION: &str = "call_auction_session";
const CONTRACT_CALL_AUCTION_ENTRYPOINT_CONTRACT: &str = "call_auction_contract";

#[ignore]
#[test]
fn should_fail_to_call_auction_from_stored_session_code() {
    ...
    let call_auction_request = ExecuteRequestBuilder::versioned_contract_call_by_name(
        *DEFAULT_ACCOUNT_ADDR,
        PACKAGE_NAME,
        None,
        CONTRACT_CALL_AUCTION_ENTRYPOINT_SESSION,
        runtime_args! {
    ...
    match builder.get_error() {
        ...
        Some(engine_state::Error::Exec(execution::Error::Revert(ApiError::AuctionError(16)))) => {}
...
#[ignore]
#[test]
fn should_call_auction_from_stored_contract_code() {
    let default_public_key_arg = DEFAULT_ACCOUNT_PUBLIC_KEY.clone();
    ...
    let call_auction_request = ExecuteRequestBuilder::versioned_contract_call_by_name(
        *DEFAULT_ACCOUNT_ADDR,
        PACKAGE_NAME,
        None,
        CONTRACT_CALL_AUCTION_ENTRYPOINT_CONTRACT,
        runtime_args! {
            auction::ARG_PUBLIC_KEY => default_public_key_arg,
        },
    )
    ...
    builder.exec(call_auction_request).commit().expect_success();
}
